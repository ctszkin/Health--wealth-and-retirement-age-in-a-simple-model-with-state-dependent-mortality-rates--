function result = agentDecision(state,par,critical_point)      
        lifeSpan=sum(state);
%       result: age health status e a c
        result=nan(lifeSpan+1,6);
        result(:,1)=1:(lifeSpan+1);
        result(1: state(1),2)=0;
        result(state(1)+1: state(2)+state(1),2)=1;
        result(lifeSpan+1,2)=2;
...
%                 if result(j,5) <
%                 critical_point_healthy_schooling(result(j,4)+1)
                if result(j,5) < critical_point(3,result(j,4)+1)
                    result(j+1,5) = h_hs(result(j,5),result(j,4)+1);
                    result(j,3) = 0;
                    if result(j,4) < par.max_edu
                        result(j+1,4) = result(j,4)+1;
                    else
                         result(j+1,4) = result(j,4);
                    end
%                 elseif result(j,5) >=
%                 critical_point_healthy_retirement(result(j,4)+1)
                elseif result(j,5) >= critical_point(4,result(j,4)+1) 
...
%                 if result(j,5) < critical_point_morbid_schooling(result(j,4)+1)
                 if result(j,5) < critical_point(1,result(j,4)+1)
...
%                 elseif result(1,5) >=
%                 critical_point_morbid_retirement(result(j,4)+1)
                elseif result(1,5) >= critical_point(2,result(j,4)+1)
